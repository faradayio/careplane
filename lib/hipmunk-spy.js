// due to plugin architecture, spy gets loaded in plugin's context as well as window context via script tag
if((typeof chrome == 'undefined' || !chrome.extension) &&
  ...
  (function() {
    ...
    var careplaneInterval = setInterval(function() {
      if($ && $.hipmunk) {
        var tabs = [];
        for(var i in $.hipmunk.url.tabs) {
          var tab = $.hipmunk.url.tabs[i];
          if(tab.flight_data) {
            tabs.push(tab);
          }
        }
