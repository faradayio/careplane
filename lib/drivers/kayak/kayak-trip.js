...
KayakTrip.prototype.init = function() {
  ...
  this.footprintView = new KayakTripFootprintView(this.$, this.tripElement);
  this.infoView = new KayakTripInfoView(this.$, this.tripElement);
...
KayakTrip.events = {
  tripDetailsSuccess: function(trip, success) {
    return function(response) {
      ...
      var div = trip.$(trip.doc.createElement('div'));
      div.addClass('careplane-trip-details');
      div.html(result.message);
      div.css('display', 'none');
      trip.$(trip.tripElement).append(div);

      trip.flights = KayakFlight.parse(trip.$, trip.$('.inlineflightitinerarylegs tr', trip.tripElement));
      success(trip);
    };
  }
};
