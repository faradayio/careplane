...
BingTrip.events = {
  tripDetailsSuccess: function(trip, success) {
    return function(result) {
      var div = bonzo.create('div');
      div.addClass('careplane-trip-details');
      div.html(result.message);
      div.css('display', 'none');
      bonzo(trip.tripElement).append(div);

      trip.flights = BingFlight.parse(trip.$('.inlineflightitinerarylegs tr', trip.tripElement));
      success(trip);
    };
  }
};
...
BingTrip.prototype.pricingSignature = function() {
  if(!this._pricingSignature) {
    var a = this.$('#carrierTransfer_' + this.id);
