...
vows.describe('BingFlight').addBatch({
  '.parse': {
    ...
    'parses a standard set of flights': function(searchData) {
      ...
      assert.equal(flights[0].airline, 'US');
      assert.equal(flights[0].origin, 'DTW');
      assert.equal(flights[0].destination, 'CLT');

      assert.equal(flights[1].airline, 'US');
      assert.equal(flights[1].origin, 'CLT');
      assert.equal(flights[1].destination, 'SFO');

      assert.equal(flights[2].airline, 'US');
      assert.equal(flights[2].origin, 'SFO');
      assert.equal(flights[2].destination, 'PHX');

      assert.equal(flights[3].airline, 'US');
